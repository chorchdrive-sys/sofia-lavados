...
async function inicializar() {
  ...
  for (const cli of clientes) {
    ...
    if (cli.barrio && cli.nroCliente && cli.nroCliente.startsWith("GEN")) {
      const nuevoCodigo = codigoBarrio(cli.barrio);
      const numeroSecuencial = cli.nroCliente.split("-")[1] || "001";
      updates.nroCliente = `${nuevoCodigo}-${numeroSecuencial}`;
      necesitaUpdate = true;
    }
    // También actualizar campo codigo si existe
    ...
    if (necesitaUpdate) {
      ...
      console.log(`✅ Actualizado: ${cli.nombre} - Código: ${updates.nroCliente || updates.codigo || "sin cambio"} | Teléfono: ${updates.telefono === "" ? "limpiado" : "ok"}`);
    }
  }
...
// ═══════════════════════════════════════════════════════════════
//  MODAL RENDICIÓN (separado del cobro)
// ═══════════════════════════════════════════════════════════════
function ModalRendicion({turno,onRegistrar,onClose}) {
  ...
  return <Modal titulo="💸 Rendir dinero" onClose={onClose}>
    <div style={{display:"flex",flexDirection:"column",gap:10,fontSize:12}}>
      ...
      <div style={{display:"flex",gap:8,marginTop:4}}>
        <Btn ghost onClick={onClose} style={{flex:1}}>Cancelar</Btn>
        <Btn full color="#16a34a" onClick={()=>{setLoading(true);onRegistrar(turno);setLoading(false);}} disabled={loading}>
...
// ═══════════════════════════════════════════════════════════════
//  MODAL OPERACION LAVADOR (con devolución)
// ═══════════════════════════════════════════════════════════════
...
export default function SofiaV4() {
  ...
  async function inicializar() {
    ...
    try {
      ...
      // === CORRECCIÓN 1 y 2: Actualizar códigos GEN y limpiar teléfonos ficticios ===
      let actualizados = 0;
      for (const c of cl) {
        let necesitaUpdate = false;
        const updates = {};
        
        // Limpiar teléfono ficticio
        if (c.telefono && String(c.telefono).startsWith("1100000")) {
          updates.telefono = "";
          necesitaUpdate = true;
        }
        
        // Actualizar código si es GEN
  ...
  // === CORRECCIÓN 5 y 8: Función para rendir turno ===
  ...
  // === CORRECCIÓN 7: Registrar operación con devolución ===
  async function registrarOperacion(lavador, monto, motivo) {
    ...
    const esDevuelve = motivo.includes("paga");
    ...
    if (esRecibe) {
      ...
      nuevoSaldo = Math.max(0, saldoActual - monto);
    }
    ...
    const reg={hora:new Date().toLocaleTimeString("es-AR",{hour:"2-digit",minute:"2-digit"}),staffNombre:lavador.nombre,clienteNombre:"—",direccion:"Operación interna",autos:0,tamano:"—",precio:monto,precioEsperado:0,metodo:"efectivo",estadoPago:esRecibe?"⚠️ Debe":esDevuelve?"✅ Pagado":"Otro",diferencia:esRecibe?monto:-monto,motivo:motivo||"Operación",fecha:diaHoy,ts:new Date().toLocaleTimeString("es-AR",{hour:"2-digit",minute:"2-digit"})};
    await fsAdd(`cierre_${diaHoy}`,reg);
    setRegistros(prev=>[...prev,{id:Date.now(),...reg}]);
        ...
        {vista==="turno"&&(
          <div className="fade layout-turno" style={{display:"grid",gridTemplateColumns:"300px 1fr",gap:16,alignItems:"start"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              ...
              {/* ✦ SUGERENCIAS - ARRIBA DE LA GRILLA (ya está arriba) */}
