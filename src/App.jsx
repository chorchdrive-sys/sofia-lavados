...
// P0-3 & P0-5 FIX: Slots ocupados robustos
...
// P0-5 FIX: Franjas dinámicas integradas
...
📅 Fecha: ${fechaAR(hoy())}`;
  ...
  // P1-2 FIX: WhatsApp automático con número del lavador
      ...
      {modo === "detalle" ? (
        <div style={{display:"flex", flexDirection:"column", gap:14}}>
          ...
          <div style={{display:"flex", flexDirection:"column", gap:6}}>
            {/* P1-3 FIX: Botones financieros solo en modo oculto */}
...
function ModalConfigCompleta({ config, onGuardar, onClose, mostrarToast, modoOculto }) {
  ...
  // P1-1 FIX: Backup Nube y Restaurar
  ...
  return (
    <Modal titulo="⚙️ Configuración General" onClose={onClose} wide>
      <div style={{display:"flex", flexDirection:"column", gap:16}}>
        ...
        {/* P1-1 FIX: Backup Nube Silencioso */}
...
function ModalNuevoTurno({ onClose, clientes, staff, turnos, asistencias, COL_TURNOS, COL_CLIENTES, mostrarToast, clientePreseleccionado, onClienteCreated, onTurnoCreado, codigosExistentes, config, celdaPreseleccionada }) {
  ...
  // P0-3 FIX: Excluir turnos cancelados del cálculo de disponibilidad
  const opcionesLavadores = useMemo(() => {
    return presentes.sort((a,b)=>(a.nombre||"").localeCompare(b.nombre||"")).map(s => {
      const ocupado = turnos.some(t => {
        ...
        if (t.lavadorId !== s.id) return false;
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))", gap:10 }}>
        {staff.map(s => {
          ...
          const turnosHoy = turnos.filter(t => t.lavadorId === s.id).length;
          // UX-3 FIX: Semáforo simplificado
          ...
          return (
            <div key={s.id} style={{
              ...
              {/* UX-3 FIX: Badge de estado claro */}
              <div style={{
              ...
              {/* P1-3 FIX: Saldo y Operación solo en modo oculto */}
              {modoOculto && (
                <>
  ...
  const renderTarjeta = (t) => {
    ...
    return (
      <div key={t.id} style={{
        ...
        {/* UX-4 FIX: Botones claros para Lluvia */}
...
function TabAgenda({ turnos, staff, asistencias, clientes, mostrarToast, onCeldaClick, onTurnoClick, onCambiarEstado, onCerrarTurno, onExportar }) {
  ...
  // P0-3 FIX: Excluir turnos cancelados del mapa de ocupación
  const turnosMap = {};
  turnos.forEach(t => {
    // P0-3 FIX: Excluir turnos cancelados para que liberen el slot visualmente
          ...
          <div style={{
            ...
            {presentes.map(s => (
              <div key={s.id} style={{
                ...
                {/* P0-2 FIX: Nombre lavador visible */}
                <div style={{fontSize:12, fontWeight:700, color:"#1e293b"}}>{s.nombre}</div>
                <div style={{fontSize:10, color:"#6b7280"}}>{s.transporte}</div>
              </div>
            ))}
                    ...
                    if (turno) {
                      ...
                      return (
                        <div key={`${s.id}-${hora}`} onClick={() => onTurnoClick(turno)}
                          ...
                          {/* P0-2 FIX: Nombre lavador explícito en celda ocupada */}
...
// ═══════════════════════════════════════════════════════════════
//  TAB CIERRE / CAJA COMPLETA - CON MODO OCULTO
// ═══════════════════════════════════════════════════════════════
function TabCierre({ registros, turnos, clientes, staff, diaHoy, mostrarToast, onPagarDeuda, onCondonarDeuda, onPunitorio, onRendir, onCobrar, onEditarCobro, onRendirTodo, rangoC, setRangoC, regMulti, loadMulti, modoOculto }) {
  ...
  // P1-3 FIX: Contenido financiero condicional
  if(!modoOculto) {
    return (
      <div style={{display:"flex", flexDirection:"column", gap:16}}>
...
export default function App() {
  // P0-1 FIX: Estado centralizado para Modo Oculto
  ...
  // C1 FIX: useEffect independiente con dependencia única [modoOculto]
  useEffect(() => {
    if (!modoOculto) {
  ...
  // P0-4 FIX: Reorganizar lluvia mueve turnos a huecos libres
          ...
          if (cancelarIds.includes(t.id)) {
            ...
            if(hOri*60+mOri < minutosAhora) {
              ...
              for(const h of franjasFuturas) {
                const ocupado = turnos.some(tt => tt.lavadorId === nuevoLav.id && tt.horasOcupadas?.includes(h));
      ...
      if (estado === "queda") {
        for (const t of turnosDelLav) {
          ...
          if(hOri*60+mOri < minutosAhora) {
            ...
            let nuevaHora = t.hora;
            for(const h of franjasFuturas) {
              const ocupado = turnos.some(tt => tt.lavadorId === lavId && tt.horasOcupadas?.includes(h) && tt.id !== t.id);
              if(!ocupado) { nuevaHora = h; break; }
            }
  ...
  // P1-3 FIX: Tabs condicionales (Cierre siempre visible pero contenido protegido)
  const tabsVisibles = [
    ...
    <div style={{ minHeight:"100vh", background:"#f9fafb", color:"#1e293b", fontFamily:"'Inter',system-ui,sans-serif", paddingBottom:90 }}>
      ...
      <header style={{position:"sticky", top:0, zIndex:100, background:"rgba(255,255,255,.9)", backdropFilter:"blur(16px)", borderBottom:"1px solid #e5e7eb", padding:"12px 20px", display:"flex", justifyContent:"space-between", alignItems:"center"}}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          {/* P0-1 FIX: Botón Salir Modo Oculto */}
          {modoOculto && (
      ...
      <main style={{ padding:20, maxWidth:1200, margin:"0 auto", animation:"fadeInUp .4s ease-out" }}>
        ...
        {/* P1-3 FIX: Pasar modoOculto a Cierre */}
        {tab === "cierre" && (
